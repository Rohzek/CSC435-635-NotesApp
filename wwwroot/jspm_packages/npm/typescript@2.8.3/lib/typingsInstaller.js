...
(function(process) {
  ...
  (function(ts) {
    ...
    function matchFiles(path, extensions, excludes, includes, useCaseSensitiveFileNames, currentDirectory, depth, getFileSystemEntries) {
      ...
      function visitDirectory(path, absolutePath, depth) {
        ...
        for (var _i = 0,
            _b = sort(files, compareStringsCaseSensitive); _i < _b.length; _i++) {
          var current = _b[_i];
          _loop_1(current);
        }
        if (depth !== undefined) {
        ...
        for (var _c = 0,
            _d = sort(directories, compareStringsCaseSensitive); _c < _d.length; _c++) {
          var current = _d[_c];
          var name = combinePaths(path, current);
          var absoluteName = combinePaths(absolutePath, current);
    ...
    function getBasePaths(path, includes, useCaseSensitiveFileNames) {
      ...
      if (includes) {
        ...
        var _loop_2 = function(includeBasePath) {
          if (every(basePaths, function(basePath) {
  ...
  (function(ts) {
    ...
    function setCustomPollingValues(system) {
      ...
      function getCustomPollingBasedLevels(baseVariable, defaultLevels) {
        var customLevels = getCustomLevels(baseVariable);
        return (pollingIntervalChanged || customLevels) && createPollingIntervalBasedLevels(customLevels ? __assign({}, defaultLevels, customLevels) : defaultLevels);
      }
    }
    ...
    function onWatchedFileStat(watchedFile, modifiedTime) {
      ...
      if (oldTime !== newTime) {
        ...
        var eventKind = oldTime === 0 ? FileWatcherEventKind.Created : newTime === 0 ? FileWatcherEventKind.Deleted : FileWatcherEventKind.Changed;
    ...
    function createRecursiveDirectoryWatcher(host) {
      ...
      function watchChildDirectories(parentDir, existingChildWatches, callback) {
        ...
        }) : ts.emptyArray, existingChildWatches, function(child, childWatcher) {
          return host.filePathComparer(child, childWatcher.dirName);
        }, createAndAddChildDirectoryWatcher, ts.closeFileWatcher, addChildDirectoryWatcher);
    ...
    ts.sys = (function() {
      ...
      function getNodeSystem() {
        ...
        function getWatchFile() {
          switch (tscWatchFile) {
            ...
            case "DynamicPriorityPolling":
              return createDynamicPriorityPollingWatchFile({
                getModifiedTime: getModifiedTime,
                setTimeout: setTimeout
              });
            case "UseFsEvents":
            ...
            case "UseFsEventsWithFallbackDynamicPolling":
              dynamicPollingWatchFile = createDynamicPriorityPollingWatchFile({
                getModifiedTime: getModifiedTime,
                setTimeout: setTimeout
              });
          ...
          return useNonPollingWatchers ? createNonPollingWatchFile() : function(fileName, callback) {
            return fsWatchFile(fileName, callback);
          };
        }
        function getWatchDirectory() {
          ...
          var watchDirectory = tscWatchDirectory === "RecursiveDirectoryUsingFsWatchFile" ? createWatchDirectoryUsing(fsWatchFile) : tscWatchDirectory === "RecursiveDirectoryUsingDynamicPriorityPolling" ? createWatchDirectoryUsing(dynamicPollingWatchFile || createDynamicPriorityPollingWatchFile({
            getModifiedTime: getModifiedTime,
            setTimeout: setTimeout
        ...
        function createNonPollingWatchFile() {
          ...
          function createDirectoryWatcher(dirName, dirPath) {
            var watcher = fsWatchDirectory(dirName, function(_eventName, relativeFileName) {
              var fileName = !ts.isString(relativeFileName) ? undefined : ts.getNormalizedAbsolutePath(relativeFileName, dirName);
        ...
        function createWatchFileUsingDynamicWatchFile(watchFile) {
          return function(fileName, callback, pollingInterval) {
            return fsWatch(fileName, 0, createFsWatchCallbackForFileWatcherCallback(fileName, callback), false, watchFile, pollingInterval);
          };
        }
        ...
        function createWatchDirectoryUsing(fsWatchFile) {
          return function(directoryName, callback) {
            return fsWatchFile(directoryName, function() {
        ...
        function generateDjb2Hash(data) {
          var chars = data.split("").map(function(str) {
  ...
  (function(ts) {
    ...
    function isBlockScopedContainerTopLevel(node) {
      return node.kind === 272 || node.kind === 237 || ts.isFunctionLike(node);
    }
    ...
    function getErrorSpanForNode(sourceFile, node) {
      ...
      var pos = isMissing ? errorNode.pos : ts.skipTrivia(sourceFile.text, errorNode.pos);
      if (isMissing) {
    ...
    function isPrologueDirective(node) {
      return node.kind === 214 && node.expression.kind === 9;
    }
    ...
    function getJSDocCommentRanges(node, text) {
      var commentRanges = (node.kind === 148 || node.kind === 147 || node.kind === 190 || node.kind === 191 || node.kind === 189) ? ts.concatenate(ts.getTrailingCommentRanges(text, node.pos), ts.getLeadingCommentRanges(text, node.pos)) : ts.getLeadingCommentRanges(text, node.pos);
    ...
    function isVariableDeclarationInVariableStatement(node) {
      return node.parent.kind === 231 && node.parent.parent.kind === 212;
    }
    ...
    function isObjectLiteralOrClassExpressionMethod(node) {
      return node.kind === 153 && (node.parent.kind === 182 || node.parent.kind === 203);
    }
    ...
    function isSuperProperty(node) {
      var kind = node.kind;
      return (kind === 183 || kind === 184) && node.expression.kind === 97;
    }
    ts.isSuperProperty = isSuperProperty;
    function isThisProperty(node) {
      var kind = node.kind;
      return (kind === 183 || kind === 184) && node.expression.kind === 99;
    }
    ...
    function nodeCanBeDecorated(node, parent, grandparent) {
      switch (node.kind) {
        ...
        case 156:
        case 153:
          return node.body !== undefined && parent.kind === 233;
        case 148:
          return parent.body !== undefined && (parent.kind === 154 || parent.kind === 153 || parent.kind === 156) && grandparent.kind === 233;
      }
      return false;
    }
    ...
    function isJSXTagName(node) {
      var parent = node.parent;
      if (parent.kind === 255 || parent.kind === 254 || parent.kind === 256) {
    ...
    function isInExpressionContext(node) {
      ...
      switch (parent.kind) {
        ...
        case 218:
          ...
          return (forStatement.initializer === node && forStatement.initializer.kind !== 231) || forStatement.condition === node || forStatement.incrementor === node;
        case 219:
        case 220:
          var forInStatement = parent;
          return (forInStatement.initializer === node && forInStatement.initializer.kind !== 231) || forInStatement.expression === node;
        case 188:
        case 206:
    ...
    function isJSDocIndexSignature(node) {
      return ts.isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) && node.typeName.escapedText === "Object" && node.typeArguments && node.typeArguments.length === 2 && (node.typeArguments[0].kind === 137 || node.typeArguments[0].kind === 134);
    }
    ...
    function getDeclaredJavascriptInitializer(node) {
      if (node && ts.isVariableDeclaration(node) && node.initializer) {
        return getJavascriptInitializer(node.initializer, false) || ts.isIdentifier(node.name) && getDefaultedJavascriptInitializer(node.name, node.initializer, false);
      }
    }
    ...
    function getAssignedJavascriptInitializer(node) {
      if (node && node.parent && ts.isBinaryExpression(node.parent) && node.parent.operatorToken.kind === 58) {
        ...
        return getJavascriptInitializer(node.parent.right, isPrototypeAssignment) || getDefaultedJavascriptInitializer(node.parent.left, node.parent.right, isPrototypeAssignment);
      }
    }
    ...
    function isSameEntityName(name, initializer) {
      ...
      if (ts.isIdentifier(name) && ts.isPropertyAccessExpression(initializer)) {
        return (initializer.expression.kind === 99 || ts.isIdentifier(initializer.expression) && (initializer.expression.escapedText === "window" || initializer.expression.escapedText === "self" || initializer.expression.escapedText === "global")) && isSameEntityName(name, initializer.name);
      }
    ...
    function getSpecialPropertyAssignmentKind(expr) {
      if (!isInJavaScriptFile(expr) || expr.operatorToken.kind !== 58 || !ts.isPropertyAccessExpression(expr.left)) {
        return 0;
      }
      var lhs = expr.left;
      if (lhs.expression.kind === 99) {
        ...
        if (id.escapedText === "exports" || id.escapedText === "module" && nextToLast.name.escapedText === "exports") {
          return 1;
        }
        return 5;
      }
    ...
    function isSpecialPropertyDeclaration(expr) {
      return isInJavaScriptFile(expr) && expr.parent && expr.parent.kind === 214 && !!ts.getJSDocTypeTag(expr.parent);
    }
    ...
    function isJSDocConstructSignature(node) {
      return node.kind === 280 && node.parameters.length > 0 && node.parameters[0].name && node.parameters[0].name.escapedText === "new";
    }
    ...
    function getSourceOfDefaultedAssignment(node) {
      return ts.isExpressionStatement(node) && ts.isBinaryExpression(node.expression) && getSpecialPropertyAssignmentKind(node.expression) !== 0 && ts.isBinaryExpression(node.expression.right) && node.expression.right.operatorToken.kind === 54 && node.expression.right.right;
    }
    ...
    function getSingleVariableOfVariableStatement(node) {
      return ts.isVariableStatement(node) && node.declarationList.declarations.length > 0 && node.declarationList.declarations[0];
    }
    function getNestedModuleDeclaration(node) {
      return node.kind === 237 && node.body && node.body.kind === 237 && node.body;
    }
    function getJSDocCommentsAndTags(node) {
      ...
      function getJSDocCommentsAndTagsWorker(node) {
        ...
        if (parent && parent.parent && parent.parent.parent && (getSingleInitializerOfVariableStatementOrPropertyDeclaration(parent.parent.parent) === node || getSourceOfDefaultedAssignment(parent.parent.parent))) {
          getJSDocCommentsAndTagsWorker(parent.parent.parent);
        }
        if (ts.isBinaryExpression(node) && getSpecialPropertyAssignmentKind(node) !== 0 || node.kind === 183 && node.parent && node.parent.kind === 214) {
          getJSDocCommentsAndTagsWorker(parent);
        }
        if (node.kind === 148) {
    ...
    function getHostSignatureFromJSDoc(node) {
      ...
      var decl = getSourceOfDefaultedAssignment(host) || getSourceOfAssignment(host) || getSingleInitializerOfVariableStatementOrPropertyDeclaration(host) || getSingleVariableOfVariableStatement(host) || getNestedModuleDeclaration(host) || host;
    ...
    function isLiteralComputedPropertyDeclarationName(node) {
      return (node.kind === 9 || node.kind === 8) && node.parent.kind === 146 && ts.isDeclaration(node.parent.parent);
    }
    ...
    function isAliasSymbolDeclaration(node) {
      return node.kind === 241 || node.kind === 240 || node.kind === 243 && !!node.name || node.kind === 244 || node.kind === 246 || node.kind === 250 || node.kind === 247 && exportAssignmentIsAlias(node);
    }
    ...
    function isDynamicName(name) {
      return name.kind === 146 && !isStringOrNumericLiteral(name.expression) && !isWellKnownSymbolSyntactically(name.expression);
    }
    ...
    function nodeStartsNewLexicalEnvironment(node) {
      var kind = node.kind;
      return kind === 154 || kind === 190 || kind === 232 || kind === 191 || kind === 153 || kind === 155 || kind === 156 || kind === 237 || kind === 272;
    }
    ...
    function getExpressionAssociativity(expression) {
      ...
      var hasArguments = expression.kind === 186 && expression.arguments !== undefined;
    ...
    function createDiagnosticCollection() {
      ...
      function getDiagnostics(fileName) {
        ...
        var fileDiags = ts.flatMap(filesWithDiagnostics, function(f) {
    ...
    function getAllAccessorDeclarations(declarations, accessor) {
      ...
      if (hasDynamicName(accessor)) {
        ...
        ts.forEach(declarations, function(member) {
          if ((member.kind === 155 || member.kind === 156) && hasModifier(member, 32) === hasModifier(accessor, 32)) {
    ...
    function tryGetClassExtendingExpressionWithTypeArguments(node) {
      if (node.kind === 205 && node.parent.token === 85 && ts.isClassLike(node.parent.parent)) {
        return node.parent.parent;
      }
    }
    ...
    function isExpressionWithTypeArgumentsInClassImplementsClause(node) {
      return node.kind === 205 && isEntityNameExpression(node.expression) && node.parent && node.parent.token === 108 && node.parent.parent && ts.isClassLike(node.parent.parent);
    }
    ts.isExpressionWithTypeArgumentsInClassImplementsClause = isExpressionWithTypeArgumentsInClassImplementsClause;
    function isEntityNameExpression(node) {
      return node.kind === 71 || node.kind === 183 && isEntityNameExpression(node.expression);
    }
    ts.isEntityNameExpression = isEntityNameExpression;
    function isRightSideOfQualifiedNameOrPropertyAccess(node) {
      return (node.parent.kind === 145 && node.parent.right === node) || (node.parent.kind === 183 && node.parent.name === node);
    }
    ts.isRightSideOfQualifiedNameOrPropertyAccess = isRightSideOfQualifiedNameOrPropertyAccess;
    function isEmptyObjectLiteral(expression) {
      return expression.kind === 182 && expression.properties.length === 0;
    }
    ts.isEmptyObjectLiteral = isEmptyObjectLiteral;
    function isEmptyArrayLiteral(expression) {
      return expression.kind === 181 && expression.elements.length === 0;
    }
  ...
  (function(ts) {
    ...
    function isEntityName(node) {
      var kind = node.kind;
      return kind === 145 || kind === 71;
    }
    ts.isEntityName = isEntityName;
    function isPropertyName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 9 || kind === 8 || kind === 146;
    }
    ts.isPropertyName = isPropertyName;
    function isBindingName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 178 || kind === 179;
    }
    ...
    function isClassElement(node) {
      var kind = node.kind;
      return kind === 154 || kind === 151 || kind === 153 || kind === 155 || kind === 156 || kind === 159 || kind === 210 || kind === 251;
    }
    ...
    function isTypeElement(node) {
      var kind = node.kind;
      return kind === 158 || kind === 157 || kind === 150 || kind === 152 || kind === 159 || kind === 251;
    }
    ...
    function isObjectLiteralElementLike(node) {
      var kind = node.kind;
      return kind === 268 || kind === 269 || kind === 270 || kind === 153 || kind === 155 || kind === 156 || kind === 251;
    }
    ts.isObjectLiteralElementLike = isObjectLiteralElementLike;
    function isTypeNodeKind(kind) {
      return (kind >= 160 && kind <= 177) || kind === 119 || kind === 134 || kind === 135 || kind === 122 || kind === 137 || kind === 138 || kind === 99 || kind === 105 || kind === 140 || kind === 95 || kind === 131 || kind === 205 || kind === 275 || kind === 276 || kind === 277 || kind === 278 || kind === 279 || kind === 280 || kind === 281;
    }
    ...
    function isModuleBody(node) {
      var kind = node.kind;
      return kind === 238 || kind === 237 || kind === 71;
    }
    ts.isModuleBody = isModuleBody;
    function isNamespaceBody(node) {
      var kind = node.kind;
      return kind === 238 || kind === 237;
    }
    ts.isNamespaceBody = isNamespaceBody;
    function isJSDocNamespaceBody(node) {
      var kind = node.kind;
      return kind === 71 || kind === 237;
    }
    ts.isJSDocNamespaceBody = isJSDocNamespaceBody;
    function isNamedImportBindings(node) {
      var kind = node.kind;
      return kind === 245 || kind === 244;
    }
    ...
    function isDeclarationKind(kind) {
      return kind === 191 || kind === 180 || kind === 233 || kind === 203 || kind === 154 || kind === 236 || kind === 271 || kind === 250 || kind === 232 || kind === 190 || kind === 155 || kind === 243 || kind === 241 || kind === 246 || kind === 234 || kind === 260 || kind === 153 || kind === 152 || kind === 237 || kind === 240 || kind === 244 || kind === 148 || kind === 268 || kind === 151 || kind === 150 || kind === 156 || kind === 269 || kind === 235 || kind === 147 || kind === 230 || kind === 291;
    }
    function isDeclarationStatementKind(kind) {
      return kind === 232 || kind === 251 || kind === 233 || kind === 234 || kind === 235 || kind === 236 || kind === 237 || kind === 242 || kind === 241 || kind === 248 || kind === 247 || kind === 240;
    }
    function isStatementKindButNotDeclarationKind(kind) {
      return kind === 222 || kind === 221 || kind === 229 || kind === 216 || kind === 214 || kind === 213 || kind === 219 || kind === 220 || kind === 218 || kind === 215 || kind === 226 || kind === 223 || kind === 225 || kind === 227 || kind === 228 || kind === 212 || kind === 217 || kind === 224 || kind === 294 || kind === 298 || kind === 297;
    }
    ...
    function isModuleReference(node) {
      var kind = node.kind;
      return kind === 252 || kind === 145 || kind === 71;
    }
    ts.isModuleReference = isModuleReference;
    function isJsxTagNameExpression(node) {
      var kind = node.kind;
      return kind === 99 || kind === 71 || kind === 183;
    }
    ts.isJsxTagNameExpression = isJsxTagNameExpression;
    function isJsxChild(node) {
      var kind = node.kind;
      return kind === 253 || kind === 263 || kind === 254 || kind === 10 || kind === 257;
    }
    ts.isJsxChild = isJsxChild;
    function isJsxAttributeLike(node) {
      var kind = node.kind;
      return kind === 260 || kind === 262;
    }
    ts.isJsxAttributeLike = isJsxAttributeLike;
    function isStringLiteralOrJsxExpression(node) {
      var kind = node.kind;
      return kind === 9 || kind === 263;
    }
    ts.isStringLiteralOrJsxExpression = isStringLiteralOrJsxExpression;
    function isJsxOpeningLikeElement(node) {
      var kind = node.kind;
      return kind === 255 || kind === 254;
    }
    ts.isJsxOpeningLikeElement = isJsxOpeningLikeElement;
    function isCaseOrDefaultClause(node) {
      var kind = node.kind;
      return kind === 264 || kind === 265;
    }
  ...
  (function(ts) {
    ...
    (function(Parser) {
      ...
      function createSourceFile(fileName, languageVersion, scriptKind, isDeclarationFile) {
        var sourceFile = new SourceFileConstructor(272, 0, sourceText.length);
        nodeCount++;
      ...
      function parseExpectedToken(t, diagnosticMessage, arg0) {
        return parseOptionalToken(t) || createMissingNode(t, false, diagnosticMessage || ts.Diagnostics._0_expected, arg0 || ts.tokenToString(t));
      }
      ...
      function parseJSDocFunctionType() {
        if (lookAhead(nextTokenIsOpenParen)) {
          var result = createNodeWithJSDoc(280);
          nextToken();
      ...
      function parseConditionalExpressionRest(leftOperand) {
        ...
        var node = createNode(199, leftOperand.pos);
      ...
      function makeBinaryExpression(left, operatorToken, right) {
        var node = createNode(198, left.pos);
        node.left = left;
      ...
      function makeAsExpression(left, right) {
        var node = createNode(206, left.pos);
      ...
      function parseUpdateExpression() {
        ...
        if ((token() === 43 || token() === 44) && !scanner.hasPrecedingLineBreak()) {
          var node = createNode(197, expression.pos);
      ...
      function parseJsxElementOrSelfClosingElementOrFragment(inExpressionContext) {
        ...
        if (opening.kind === 255) {
          ...
          var node = createNode(257, opening.pos);
        ...
        if (inExpressionContext && token() === 27) {
          ...
          if (invalidElement) {
            ...
            var badNode = createNode(198, result.pos);
      ...
      function parseJsxAttributes() {
        var jsxAttributes = createNode(261);
      ...
      function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext) {
        ...
        parseExpected(27);
        if (token() === 29) {
          var node_1 = createNode(258, fullStart);
          scanJsxText();
      ...
      function parseJsxElementName() {
        ...
        while (parseOptional(23)) {
          var propertyAccess = createNode(183, expression.pos);
      ...
      function parseMemberExpressionRest(expression) {
        while (true) {
          ...
          if (token() === 51 && !scanner.hasPrecedingLineBreak()) {
            nextToken();
            var nonNullExpression = createNode(207, expression.pos);
            nonNullExpression.expression = expression;
            expression = finishNode(nonNullExpression);
            continue;
          }
          if (!inDecoratorContext() && parseOptional(21)) {
            var indexedAccess = createNode(184, expression.pos);
          ...
          if (token() === 13 || token() === 14) {
            var tagExpression = createNode(187, expression.pos);
      ...
      function parseCallExpressionRest(expression) {
        while (true) {
          ...
          if (token() === 27) {
            ...
            var callExpr = createNode(185, expression.pos);
      ...
      function parseParenthesizedExpression() {
        var node = createNodeWithJSDoc(189);
        parseExpected(19);
        node.expression = allowInAnd(parseExpression);
        parseExpected(20);
        return finishNode(node);
      ...
      function parseArgumentOrArrayLiteralElement() {
        return token() === 24 ? parseSpreadElement() : token() === 26 ? createNode(204) : parseAssignmentExpressionOrHigher();
      }
      ...
      function parseNewExpression() {
        ...
        parseExpected(94);
        if (parseOptional(23)) {
          var node_2 = createNode(208, fullStart);
      ...
      function parseForOrForInOrForOfStatement() {
        ...
        if (awaitToken ? parseExpected(144) : parseOptional(144)) {
          var forOfStatement = createNode(220, pos);
          ...
          var forInStatement = createNode(219, pos);
          forInStatement.initializer = initializer;
          forInStatement.expression = allowInAnd(parseExpression);
          parseExpected(20);
          forOrForInOrForOfStatement = forInStatement;
        } else {
          var forStatement = createNode(218, pos);
        ...
        switch (token()) {
          ...
          default:
            if (node.decorators || node.modifiers) {
              var missing = createMissingNode(251, true, ts.Diagnostics.Declaration_expected);
      ...
      function parseEnumMember() {
        var node = createNodeWithJSDoc(271);
        node.name = parsePropertyName();
      ...
      function isExternalModuleReference() {
        return token() === 133 && lookAhead(nextTokenIsOpenParen);
      }
      ...
      function parseImportClause(identifier, fullStart) {
        var importClause = createNode(243, fullStart);
        if (identifier) {
      ...
      function parseNamespaceImport() {
        var namespaceImport = createNode(244);
        parseExpected(39);
        parseExpected(118);
      ...
      function setExternalModuleIndicator(sourceFile) {
        sourceFile.externalModuleIndicator = ts.forEach(sourceFile.statements, function(node) {
          return ts.hasModifier(node, 1) || node.kind === 241 && node.moduleReference.kind === 252 || node.kind === 242 || node.kind === 247 || node.kind === 248 ? node : undefined;
        });
      }
      var JSDocParser;
      (function(JSDocParser) {
        ...
        function parseJSDocTypeExpression(mayOmitBraces) {
          var result = createNode(274, scanner.getTokenPos());
        ...
        function parseJSDocCommentWorker(start, length) {
          ...
          function parseParameterOrPropertyTag(atToken, tagName, target) {
            ...
            var result = target === 1 ? createNode(287, atToken.pos) : createNode(292, atToken.pos);
          ...
          function parseNestedTypeLiteral(typeExpression, name) {
            if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
              var typeLiteralExpression = createNode(274, scanner.getTokenPos());
              var child = void 0;
              ...
              if (children) {
                jsdocTypeLiteral = createNode(283, start_2);
          ...
          function parsePropertyAccessEntityNameExpression() {
            ...
            while (parseOptional(23)) {
              var prop = createNode(183, node.pos);
          ...
          function parseTypedefTag(atToken, tagName) {
            ...
            if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
              ...
              while (child = tryParse(function() {
                ...
                if (!jsdocTypeLiteral) {
                  jsdocTypeLiteral = createNode(283, start_3);
                }
                if (child.kind === 289) {
                  if (childTypeTag) {
                    break;
            ...
            function parseJSDocTypeNameWithNamespace(flags) {
              ...
              if (typeNameOrNamespaceName && parseOptional(23)) {
                var jsDocNamespaceNode = createNode(237, pos);
              ...
              switch (nextJSDocToken()) {
                case 57:
                  if (canParseTag) {
                    ...
                    if (child && child.kind === 287 && (ts.isIdentifier(child.name) || !escapedTextsEqual(name, child.name.left))) {
                      return false;
                    }
                    return child;
                  }
        ...
        function visitNode(child) {
          ...
          if (fullEnd >= changeStart) {
            ...
            if (ts.hasJSDocNodes(child)) {
              for (var _i = 0,
                  _a = child.jsDoc; _i < _a.length; _i++) {
                var jsDocComment = _a[_i];
                visitNode(jsDocComment);
              }
            }
    ...
    function processCommentPragmas(context, sourceText) {
      ...
      for (var _i = 0,
          pragmas_1 = pragmas; _i < pragmas_1.length; _i++) {
      ...
      context.pragmas.forEach(function(entryOrList, key) {
        switch (key) {
              ...
              ts.forEach(ts.toArray(entryOrList), function(arg) {
                if (arg.arguments["no-default-lib"]) {
                  ...
                  typeReferenceDirectives_1.push({
                    pos: arg.arguments.types.pos,
                    end: arg.arguments.types.end,
                    fileName: arg.arguments.types.value
                  });
                } else if (arg.arguments.path) {
                  referencedFiles_1.push({
                    pos: arg.arguments.path.pos,
                    end: arg.arguments.path.end,
                    fileName: arg.arguments.path.value
                  });
                } else {
          ...
          case "amd-module":
            {
              if (entryOrList instanceof Array) {
                for (var _i = 0,
                    entryOrList_1 = entryOrList; _i < entryOrList_1.length; _i++) {
          ...
          case "jsx":
            return;
          default:
            ts.Debug.fail("Unhandled pragma kind");
        }
      });
    }
    ...
    function extractPragmas(pragmas, range, text) {
      ...
      if (tripleSlash) {
        ...
        if (pragma.args) {
          var argument = {};
          for (var _i = 0,
              _a = pragma.args; _i < _a.length; _i++) {
            var arg = _a[_i];
  ...
  (function(ts) {
    ...
    var Semver = (function() {
      function Semver(major, minor, patch, isPrerelease) {
        this.major = major;
        this.minor = minor;
        this.patch = patch;
      ...
      Semver.prototype.greaterThan = function(sem) {
        return this.major > sem.major || this.major === sem.major && (this.minor > sem.minor || this.minor === sem.minor && (!this.isPrerelease && sem.isPrerelease || this.isPrerelease === sem.isPrerelease && this.patch > sem.patch));
      };
      return Semver;
    }());
    ts.Semver = Semver;
      ...
      (function(typingsInstaller) {
        ...
        var TypingsInstaller = (function() {
          ...
          TypingsInstaller.prototype.install = function(req) {
            ...
            }) : undefined, req.fileNames, req.projectRootPath, this.safeList, this.packageNameToTypingLocation, req.typeAcquisition, req.unresolvedImports, this.typesRegistry);
            if (this.log.isEnabled()) {
          ...
          TypingsInstaller.prototype.processCacheLocation = function(cacheLocation) {
            ...
            if (this.installTypingHost.fileExists(packageJson) && this.installTypingHost.fileExists(packageLockJson)) {
              ...
              var npmLock = JSON.parse(this.installTypingHost.readFile(packageLockJson));
              if (this.log.isEnabled()) {
              ...
              if (npmConfig.devDependencies && npmLock.dependencies) {
                for (var key in npmConfig.devDependencies) {
                  ...
                  var semver = ts.Semver.parse(version_1);
                  var newTyping = {
          ...
          TypingsInstaller.prototype.watchFiles = function(projectName, files) {
            ...
            ts.mutateMap(watchers, ts.arrayToSet(files), {
              createNewValue: function(file) {
                ...
                return isLoggingEnabled ? {close: function() {
